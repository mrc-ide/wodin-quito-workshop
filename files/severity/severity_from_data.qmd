...
There are several types of outbreak surveillance data that can be used to calculate severity. For this extremely simple calculation we do not anything other than the cumulative numbers up to the present day.

That said, two common outbreak surveillance data streams are aggregated time-series data and linelist data. Aggregated time-series data is basically a count of severity outcomes (hospitalisations and deaths, in our example) per unit-time. The reporting unit-time can be days, weeks or months (even years could make sense for chronic diseases), and the counts per unit-time can in turn correspond to incident (e.g., daily new cases) or cumulative outcomes up to that point in time.
...
Once again, we see that the simple HFR calculation underestimates the *true* HFR, which for the sake of the example we are plotting here from the final outbreak data. This cannot be known at the start of an outbreak, however, and whilst the calculation of a binomial confidence interval adds some information on the data available to make our simple calculation, it is does not do a good job at capturing the true HFR.
:::
...
The problems with the previous estimator were caused by cases being counted before they had transitioned into deaths. We can avoid this by only counting "closed" cases, i.e., those for which we know the final outcome. This is only possible if we have a measure of the total number of recoveries in the population, which may not always be true.
:::
...
The new estimate appears to be less biased, but has much greater uncertainty. This is because we are only making use of information in hospitalisations that have completed their journey to death or recovery, which in the early stages of an outbreak will be a small minority of cases.

There are also additiona limitations of this approach that are worth noting. For one, if the average time to death is very different from the average time to recovery then estimates will be biased. It may also be difficult to obtain unbiased estimates of the actual number of recoveries as surveillance tends to be biased towards better recording of the more severe outcomes.
:::
...
These distributions are quite noisy, especially deaths given that there have been relatively few "closed" hospitalisations to date. However, even with this limited data we can get a picture of the overall mean and spread. We also get a picture of the proportion of closed hospitalised cases that have resulted in death versus recovery.

The time from hospitalisation to recovery is quite variable, ranging from 0 to 28 days, with a mean around 12 days post-hospitalisation. Although there are fewer deaths, the observed time from hospitalisation to death ranged from 1 to 29 days, with a mean of 16 days post-hospitalisations.
:::

In our simulated data up to day 30 of the outbreak, 391 hospitalisations remain open. Estimates relying solely on closed cases may be biased. By dealing with censoring we can make use of the additional information from open cases as well - potentially leading to more accurate estimates of key parameters like the HFR. Two methods for doing this include the use of **parametric mixture models**, which we will not discuss here, and the **Kaplan-Meier method** which we will!
...
::: {.callout-note icon="false"}
## Question 8: What does the gap between the lines represent? Should we have more data, do you think the lines would eventually intersect and, if so, what would the intersection represent?

The vertical gap between the lines indicates the proportion of cases without a recorded outcome. Once the epidemic is over and all cases have reached an outcome, the two lines will intersect. The point of intersection will be an unbiased estimate of the HFR. While the epidemic is ongoing, our estimate of the HFR must lie between these two lines. We can arrive at a point estimate by assuming that the relative probabilities of death versus recovery among open cases are the same as those observed in closed cases up to the time of the analysis.

There is currently no known R package for performing this analysis, so we will not show an illustrative example here, but the approach is implemented in the [STATA package casefat](https://econpapers.repec.org/scripts/search.pf?ft=casefat).
:::
...
Let's see what we can learn from more aggregate population-level time series data and how we can explicitly account for delays and censoring. We will once again use our daily incidence data, however we will no use the entire time series. One important factor for this analysis is that we must have uninterrupted time series data, i.e. we cannot have any dates with missing data. If we do have missing data, then we should first impute those values before carrying out this analysis.
...
Looking at the incidence data, we can see a clear lag between the hospitalisations and deaths curve. The `cfr` package in R takes this lag into account when estimating the CFR. It does so by **estimating the number of cases that are still open** based on the incidence data and an **assumed delay distribution between cases and deaths**. It then estimates the CFR based on the estimated number of closed cases, which avoids some of the biases we saw earlier in our simple (crude) calculations.

Let's start with a delay distribution representing the probability that a hospitalised case dies at each time interval after symptom onset. In our example, we will assume a **parametric** form for this distribution - a [gamma distribution](https://en.wikipedia.org/wiki/Gamma_distribution). When doing this in a real-time outbreak response, we may not always have access to linelist data to estimate the **mean** and **standard deviation** of our gamma distribution, so we may have to make assumptions (e.g., based on knowledge from previous/similar outbreaks). Here, we will get these parameters from our data.
...
In this example, the `cfr_static` function is estimating an HFR of 33.3% (95%CI 32.7-33.9), compared to our *true* estimate of 33.3% based on total cumulative hospitalisations and deaths at the end of the outbreak. So, not bad at all! Admittedly, the estimates are not different because we have information past the peak of the outbreak wave and there are very few open hospitalised cases left.

Let's see, however, what would happen if we compute this estimate **throughout** the course of the outbreak with data available up to a present time, compared to our simple method based on cumulative counts:
...
We can see that `cfr_static()` begins producing reliable estimates at a much earlier point in the epidemic due to accounting for open cases.

This analysis assumes we know the correct delay distribution, i.e. the value of 16 days is exactly correct for this dataset (we know this because the data were simulated). **In reality we will not know this distribution perfectly**, and will have to estimate it from data. In general, we are better off accounting for delays even if this distribution is imperfect rather than ignoring delays completely.


## Estimating delays and CFR together

Rather than assuming a known delay distribution, we can **estimate this distribution directly from the data**. If the time series of deaths looks like a lagged version of the time series of cases, then this lag can tell us the average time from case to death. Similarly, if the time series of deaths looks like a more spread out version of the time series of cases, then this tells us something about the variation in time from case to death. We can combine these ideas into a mathematical model. As above, we assume a simple parametric form for the delay distribution, defined in terms of a mean and standard deviation.

Although there are no out-of-the-box solutions for fitting this model, there are many tools that we could use to perform model fitting. In this example, we will use [drjacoby](https://mrc-ide.github.io/drjacoby/), which performs **Bayesian inference via Markov chain Mote Carlo (MCMC)**. The Bayesian approach requires us to define priors on the unknown parameters, which are distributions that capture what we believe about the parameters *before* we have seen any data. Alongside incorporating prior knowledge, the Bayesian framework allows us to:

-   **Estimate the CFR**, along with the delay distribution's **mean** and **standard deviation**
-   **Quantify uncertainty** around all parameter estimates

For this example, we will use the following weakly informative priors:

-   **CFR**: Uniform prior that is equally likely to take any value between 0 and 1
...
df_params <- define_params(name = "HFR", min = 0, max = 1,
                           name = "delay_death_mean", min = 0, max = Inf,
                           name = "delay_death_sd", min = 0, max = Inf)
...
logprior <- function(params, misc) {
  ...
  ret <- dunif(HFR, min = 0, max = 1, log = TRUE) +
    dexp(delay_death_mean, rate = 1 / 30, log = TRUE) +
    dexp(delay_death_sd, rate = 1 / 10, log = TRUE)
...
There are many steps involved in running an MCMC and checking that results are reliable. We will not dwell on these steps here, but instead will focus on the results:
...
We obtain 95% credible intervals on the CFR, and also on the mean and standard deviation of the delay distribution. Unlike the previous analysis, any uncertainty in the parameters of the delay distribution are taken into account in our estimate of the CFR. Results are similar to the previous analysis, but we have had to make fewer assumptions.

### Summary

-   The `cfr` package accounts for delays between cases and deaths to provide more accurate CFR estimates by adjusting for open cases.
-   Bayesian models, such as MCMC, offer a flexible framework for jointly estimating the CFR and the delay distribution, incorporating uncertainty in both.